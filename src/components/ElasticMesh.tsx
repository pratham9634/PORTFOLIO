...
export const ElasticMesh: React.FC<ElasticMeshProps> = ({
  ...
  useEffect(() => {
    ...
    function toPlane(clientX: number, clientY: number) {
      if (!container) return;
      const currentFit = propsRef.current.fit || 1.05;
      ...
      const ct = Math.cos(t);
      const st = Math.sin(t);
      const a = clipY / (ct * currentFit * DIST);
      const py = (a * DIST) / (1 + a * st);
      const persp = DIST / (DIST - py * st);
      pointer.tx = (clipX * aspect) / (persp * currentFit);
      pointer.ty = py;
    }
    ...
    function frame(now: number) {
      ...
      program.uniforms.uFullBleed.value = p.fullBleed ? 1 : 0;
